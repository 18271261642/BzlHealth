...
public class CusFriendSleepView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if(sleepList.size() <=0){
             ...
             */
            //每一分钟的宽度
            float mCurrWidth = mWidth / allSleepTime;
            ...
            for(int i = 0;i<sleepList.size();i++){
                ...
                xCountSum +=  sleepTime;
                if(sleepType == 1){     //清醒
                    RectF rectF = new RectF(xCountSum-sleepTime,-dp2px(160),xCountSum,0);
                    canvas.drawRect(rectF,awakePaint);
                    //Log.e(TAG,"---清醒----xCountSum--="+xCountSum+"---="+(xCountSum-sleepTime));
                }else if(sleepType == 2){   //浅睡
                    RectF rectF = new RectF(xCountSum-sleepTime,-dp2px(130),xCountSum,0);
                    canvas.drawRect(rectF,lowPaint);

                }else if(sleepType == 3){   //深睡
                    RectF rectF = new RectF(xCountSum-sleepTime,-dp2px(80),xCountSum,0);
            ...
            if(isShowSeekLin){
                //绘制一条白线
                RectF linRectF = new RectF(seekX * mCurrWidth,-dp2px(160),seekX * mCurrWidth+10,0);
                ...
                //绘制显示的时间
                canvas.drawText(timeTxt,seekX<=sleepList.size()/2?seekX * mCurrWidth+mCurrWidth+10:seekX * mCurrWidth-mCurrWidth-10,
