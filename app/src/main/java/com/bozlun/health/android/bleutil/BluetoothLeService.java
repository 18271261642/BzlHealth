...
import com.bozlun.health.android.bean.B15PSleepBean;
...
import com.bozlun.health.android.bean.SleepCurveBean;
...
import static com.bozlun.health.android.bleutil.Customdata.byteToHex;
import static com.bozlun.health.android.bleutil.Customdata.huansuan;
...
//                sendMeassage(ReadSteps, String.valueOf(step));
                ...
                VerifyUtil.sendEventBus(MyApp.getContext(), step,b15sKcal);
...
/***************************b15p*******************************/
               ...
               // sendMeassage(batterylevel, String.valueOf(huansuan(Integer.parseInt(da[2], 16))));
                ...
                VerifyUtil.sendEventBus(MyApp.getContext(), step,0);
               ...
               // sendMeassage(Currentversionnumber, String.valueOf("00" + da[1] + "." + da[2] + "00"));
                ...
                if(0>=Integer.valueOf(aa)&&20>Integer.valueOf(HATA)){
                    ...
                    try{
                        map.put("date",mydat);
                        //查询登录标记
                        SharedPreferences denglu=  MyApp.getContext().getSharedPreferences("userId", Activity.MODE_PRIVATE);
                        if(null!=denglu){
                            map.put("userId",denglu.getString("userId", ""));
                        }
                        if(null!= SharedPreferencesUtils.readObject(MyApp.getContext(), Commont.BLENAME)&&null!=SharedPreferencesUtils.readObject(MyApp.getContext(),Commont.BLEMAC)) {
                            map.put("deviceCode", SharedPreferencesUtils.readObject(MyApp.getContext(),Commont.BLEMAC).toString());
                        }
                    ...
                    try{
                        if(null!=jsonArray){
                            if(jsonArray instanceof JSONArray){
                                //这里保存数据的包号
                                if(!"".equals(pakege)){
                                    SharedPreferencesUtils.saveObject(MyApp.getContext(),"mypakegeone",pakege);
                                    SharedPreferencesUtils.saveObject(MyApp.getContext(),"mypakegetwo",pakege2);
                                }
                                EventBus.getDefault().post(new MessageEvent("all_day_acyivity", alldayActivity));
                            }
                        }
                ...
                try {
                    if (da[1].equals("05")) {
                       ...
                       B15PSleepBean b15PSleepBean = new B15PSleepBean((String) map.get("startTime"), (String) map.get("endTime"), 0, (int) map.get("Deepsleep"), (int) map.get("Lightsleep"), (int) map.get("sleepquality"));
                        //根据结束时间来排除重复上传
                         List<B15PSleepBean> list = MyApp.getInstance().getDaoSession().getB15PSleepBeanDao().queryBuilder().
...
*/
    ...
    public   boolean coonected(){
       try{
           String deviceaddress = (String) SharedPreferencesUtils.readObject(this, Commont.BLEMAC);
...
//            EventBus.getDefault().post(new MessageEvent(send_tag.substring(send_tag.lastIndexOf("_") + 1), result));
            ...
            if(null!= SharedPreferencesUtils.readObject(MyApp.getContext(),Commont.BLEMAC)){
                System.out.print("连接新设备");
                connect(SharedPreferencesUtils.readObject(MyApp.getContext(),Commont.BLEMAC).toString());//连接蓝牙0,1为断开
            }
    ...
    public synchronized boolean writeRXCharacteristic(byte[] bb){
        ...
        if(null!=SharedPreferencesUtils.readObject(MyApp.getContext(),  Commont.BLENAME)){
