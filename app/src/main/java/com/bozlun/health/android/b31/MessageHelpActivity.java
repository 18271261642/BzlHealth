...
public class MessageHelpActivity extends WatchBaseActivity {
    ...
    private void readPermissions() {
        ...
        if(AndPermission.hasPermissions(MessageHelpActivity.this,Manifest.permission.READ_CONTACTS)){
            helpContPermissionTv.setText("已获取");
            helpContPermission2Tv.setText("已获取");
        }else{
            helpContPermissionTv.setText("未获取");
            helpContPermission2Tv.setText("未获取");
        }

        //拨打电话
        if(AndPermission.hasPermissions(MessageHelpActivity.this,Manifest.permission.CALL_PHONE)){
            helpPhonePermissionTv.setText("已获取");
        }else{
            helpPhonePermissionTv.setText("未获取");
        }

        //通话记录
        if(AndPermission.hasPermissions(MessageHelpActivity.this,Manifest.permission.READ_CALL_LOG)){
            helpCallLogPermissionTv.setText("已获取");
        }else {
            helpCallLogPermissionTv.setText("未获取");
        }

        //短信
        if(AndPermission.hasPermissions(MessageHelpActivity.this,Manifest.permission.READ_SMS)){
            helpSMSPermissionTv.setText("已获取");
        }else{
            helpSMSPermissionTv.setText("未获取");
        }
    ...
    private void initViews() {
        ...
        commentB30TitleTv.setText("消息帮助");
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            ...
            case R.id.helpContPermission2Tv:    //联系人
                String contPer2 = helpContPermission2Tv.getText().toString().trim();
                if(contPer2.equals("未获取")){
